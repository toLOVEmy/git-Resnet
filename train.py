...
def main():
    ...
    image_path = os.path.join(data_root, "ER_BLS", "ER-BLS", "datasets", "SCUT-FBP5500")  # flower data set path
    ...
    with open(log_file, 'w') as f:
        f.write('Fold,Epoch,Training Loss,Validation Loss,Training Accuracy,Validation Accuracy\n')
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(train_dataset)):
        ...
        train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, sampler=train_subsampler, num_workers=nw, worker_init_fn=worker_init_fn)
        validate_loader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, sampler=val_subsampler, num_workers=nw, worker_init_fn=worker_init_fn)
        ...
        epochs = 60
        best_acc = 0.0
        save_path = f'.\FBPresNet50_16_yuxian_2_fold{fold + 1}.pth'
        ...
        for epoch in range(epochs):
            ...
            val_acc = acc / len(val_idx)
            running_loss = running_loss / train_steps
            running_val_loss = running_val_loss / len(validate_loader)

            print('[fold %d, epoch %d] train_loss: %.3f  val_loss: %.3f  train_accuracy: %.3f  val_accuracy: %.3f' %
                  (fold + 1, epoch + 1, running_loss, running_val_loss, train_acc, val_acc))

            # 以CSV格式写入日志文件
            with open(log_file, 'a') as f:
                f.write(f'{fold + 1},{epoch + 1},{running_loss:.3f},{running_val_loss:.3f},{train_acc:.3f},{val_acc:.3f}\n')
